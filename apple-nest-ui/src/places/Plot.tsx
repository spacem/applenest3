...
export class Plot extends Component<PlaceProps, PlotState> {
  ...
  async plantSeed() {
    try {
      ...
      const { character: updatedCharacter, message } = await webService.plantSeed(this.props.character);
  ...
  async harvestCrop() {
    try {
      this.setState({ saving: true, error: null });
      const webService = new PlotWebservice();
      const { character: updatedCharacter, message } = await webService.harvestCrop(this.props.character);
      this.props.onUpdateCharacter(updatedCharacter);
      this.setState({ message, saving: false });
    } catch (err) {
      this.setState({ error: err, saving: false });
  ...
  render() {
    ...
    <Saving saving={this.state.saving}>
      ...
      <div>
        <button onClick={() => this.props.onChangePlace(Place.Farm)}>Back To Farm</button>
      </div>
    </Saving>
    </>;
  }
