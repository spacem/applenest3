...
import { ActionResponse, Character, Quest } from 'apple-nest-interfaces';
...
export class EventPlannerService {
  ...
  async giveReward(character: Character, currentTime: number): Promise<ActionResponse> {
    let remainingTime = 0;
    ...
    if (remainingTime > 0) {
      return {
        character,
        message: `${Math.ceil(remainingTime / 60)} minutes left until you can get another event reward`
      };
    } else {
